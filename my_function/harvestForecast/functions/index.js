 ...
 *  of an array of data for a given number of days.
 ...
 * @return {number|null} - The calculated SMA,
    ...
    .onRun(async (context) => {
      ...
      fieldsSnapshot.forEach(async (doc) => {
        ...
        if (accumulatedGdd < eightyPercentMaxGdd) {
          console.info(
              `Field ${fieldId}
               has not reached 80% of Max GDD Skipping,
                corrected AGDD right now is ${accumulatedGdd}.`);
          return;
        }
